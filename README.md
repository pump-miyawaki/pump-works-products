...
- **MENU/PLAN/PRICEセクション**（coaching / community / weeding）：`¥ --` や「商品名を入力」はすべてプレースホルダーです。実際の商品・プラン・料金に差し替えてください
- **FLOWセクション**（coaching / community / weeding）：申込みから利用開始までの流れです。実際の手順に合わせて文言を調整してください
- **FAQセクション**（全ページ共通）：「質問文を入力してください」を、実際によくある質問と回答に差し替えてください
- **coffee.html のLINKSセクション**：InstagramとオンラインストアのURL（`href="https://www.instagram.com/your_account/"` など）を実際のリンクに差し替えてください
- **coffee.html のGALLERYセクション**：`images` フォルダに `coffee-1.jpg` 〜 `coffee-6.jpg` を追加すると自動的に表示されます。写真がまだない項目は、コードは残しつつ空欄のまま表示されます（壊れた画像アイコンは出ません）
